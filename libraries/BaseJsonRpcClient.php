...
class BaseJsonRpcClient
{
	...
	public $UseObjectsInResults = TRUE;
...
class BaseJsonRpcCall
{
	...
	public function SetResult($data, $useObjects = false )
	{
		if ($useObjects)
		{
			$this->Errors 	= property_exists($data, 'error')?$data->error:NULL;
			...
			$this->Notifications	= property_exists($data, 'notifications')?$data->notifications:NULL;
			$this->Result			= property_exists($data, 'result')?$data->result:NULL;
		} else {
			$this->Errors 	= isset($data['error'])?$data['error']:NULL;
